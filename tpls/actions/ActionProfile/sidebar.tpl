	...
	<div class="panel-body">
		...
		{if $sAction=='settings' AND E::UserId() == $oUserProfile->getId()}
			...
			<p class="small upload-photo">
				<a href="#" id="foto-upload" class="link-dotted">{if E::User()->getProfilePhoto()}{$aLang.settings_profile_photo_change}{else}{$aLang.settings_profile_photo_upload}{/if}</a>&nbsp;&nbsp;&nbsp;
				<a href="#" id="foto-remove" class="link-dotted" onclick="return ls.user.removeFoto();" style="{if !E::User()->getProfilePhoto()}display:none;{/if}">{$aLang.settings_profile_foto_delete}</a>
			</p>
...
<section class="panel panel-default block block-type-profile-nav">
	<div class="panel-body">
		...
		<ul class="nav nav-pills nav-stacked">
			...
			<li {if $sAction=='profile' AND ($aParams[0]=='whois' OR $aParams[0]=='')}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}">{$aLang.user_menu_profile_whois}</a></li>
			<li {if $sAction=='profile' AND $aParams[0]=='wall'}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}wall/">{$aLang.user_menu_profile_wall}{if ($iCountWallUser)>0}<span class="badge pull-right">{$iCountWallUser}</span>{/if}</a></li>
			<li {if $sAction=='profile' AND $aParams[0]=='created'}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}created/topics/">{$aLang.user_menu_publication}{if ($iCountCreated)>0}<span class="badge pull-right">{$iCountCreated}</span>{/if}</a></li>
			<li {if $sAction=='profile' AND $aParams[0]=='favourites'}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}favourites/topics/">{$aLang.user_menu_profile_favourites}{if ($iCountFavourite)>0}<span class="badge pull-right">{$iCountFavourite}</span>{/if}</a></li>
			<li {if $sAction=='profile' AND $aParams[0]=='friends'}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}friends/">{$aLang.user_menu_profile_friends}{if ($iCountFriendsUser)>0}<span class="badge pull-right">{$iCountFriendsUser}</span>{/if}</a></li>
			<li {if $sAction=='profile' AND $aParams[0]=='stream'}class="active"{/if}><a href="{$oUserProfile->getProfileUrl()}stream/">{$aLang.user_menu_profile_stream}</a></li>
